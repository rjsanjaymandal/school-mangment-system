...
- **Admin-Only**: The ability to initiate a Shadow session is strictly restricted to users with the `admin` role. The `startImpersonation` action explicitly calls `isAdmin()` before setting the impersonation cookie.
