...
export function MessagesDashboard({ inbox, sent, contacts, currentUserId }: MessagesDashboardProps) {
    ...
    const renderMessageList = (messages: any[], type: "inbox" | "sent") => (
        <div className="divide-y divide-slate-100">
            {messages.length === 0 ? (
                <div className="p-12 text-center text-slate-400 font-medium">No messages yet.</div>
            ) : (
                messages.map((msg) => {
                    ...
                    return (
                        ...
                        >
                            <div className={cn("h-10 w-10 rounded-xl flex items-center justify-center font-bold text-white shrink-0", type === "inbox" && !msg.is_read ? "bg-blue-500 neon-blue" : "bg-slate-900")}>
                                {person?.first_name?.[0] || "?"}
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-baseline mb-1">
                                    <span className={cn("text-sm truncate", type === "inbox" && !msg.is_read ? "font-black text-slate-900" : "font-bold text-slate-700")}>
                                ...
                                <p className="text-xs font-bold text-slate-600 truncate">{msg.subject || "(No subject)"}</p>
                                <p className="text-[10px] text-slate-400 truncate mt-0.5">{msg.body}</p>
                            </div>
                        </button>
                    );
                })
    ...
    return (
        <div className="h-[calc(100vh-160px)] flex gap-x-6 animate-in fade-in duration-700">
                                ...
                                <div className="space-y-4 pt-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <textarea
                                            ...
                                            className="w-full h-32 rounded-xl border border-slate-200 p-4 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-slate-900"
                                        />
                                    </div>
                                    <Button onClick={handleSend} disabled={loading} className="w-full rounded-xl py-6 bg-slate-900 text-white font-bold gap-x-2">
            ...
            <Card className="flex-1 border-none glass futuristic-card flex flex-col overflow-hidden relative">
                ...
                {selectedMessage ? (
                    <>
                        <div className="p-6 border-b border-slate-100 bg-white/40 backdrop-blur-sm shrink-0">
                            <div className="flex items-center gap-x-4">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-xs font-bold text-slate-400">{selectedMessage.subject || "(No subject)"}</p>
                                </div>
                    ...
                    <div className="flex-1 flex items-center justify-center">
                        <div className="text-center">
                            ...
                            <p className="text-xs text-slate-400 font-medium mt-1">Choose a message from the list to read</p>
                        </div>
                    </div>
                )}
            </Card>
