...
export default function InventoryDashboardClient({ initialInventory, userRole }: InventoryDashboardClientProps) {
  ...
  return (
    <div className="p-6 space-y-8 animate-in fade-in duration-700">
        ...
        actions={
          isAdmin && (
            <div className="flex items-center gap-3">
              <button className="h-10 rounded-xl border border-slate-200 text-slate-700 font-black text-[10px] uppercase tracking-widest px-6 hover:bg-slate-50 transition-all" onClick={() => router.refresh()}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-5">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="text-lg font-black tracking-tight text-slate-900">Consumption Velocity</h3>
              <p className="text-sm text-slate-500">Stock consumption vs restock by category</p>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="text-lg font-black tracking-tight text-slate-900">Distribution</h3>
              <p className="text-sm text-slate-500">Items per category</p>
            </div>
            <Activity className="h-5 w-5 text-slate-300" />
          </div>
          <div className="h-[280px]">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl overflow-hidden">
          <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="relative flex-1 w-full max-w-xs">
              ...
              <Input placeholder="Search inventory..." className="pl-11 rounded-xl border-slate-200" value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} />
            </div>
            <div className="flex items-center gap-2">
              {statusFilter && (
                <span className="px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest bg-indigo-50 text-indigo-600 border border-indigo-200 inline-flex items-center gap-1">
                  {statusFilter}
                  <button onClick={() => { setStatusFilter(null); setCurrentPage(1); }}><X className="h-3 w-3" /></button>
                </span>
              )}
              <button className="h-10 rounded-xl border border-slate-200 text-slate-700 font-black text-[10px] uppercase tracking-widest px-4 hover:bg-slate-50 transition-all" onClick={() => setShowFilters(!showFilters)}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-slate-50/50 border-b border-slate-100">
                <tr>
                  <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500">Item</th>
                  <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500">Stock Level</th>
                  <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500">Status</th>
                  {isAdmin && <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500 text-right">Actions</th>}
                </tr>
              </thead>
                ...
                {paginatedInventory.length === 0 ? (
                  ...
                  paginatedInventory.map((item: any) => (
                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <div className="flex items-center gap-3">
                          {isAdmin && (
                            <button onClick={() => handleStockChange(item.id, -1)} disabled={processing === item.id || item.quantity_in_stock <= 0}
                              className="h-7 w-7 rounded-xl border border-slate-200 flex items-center justify-center text-slate-400 hover:bg-slate-100 transition-all disabled:opacity-30">
                      ...
                      <td className="px-4 py-5">
                        <span className={cn("px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border", item.status === "Critical" ? "bg-rose-50 text-rose-600 border-rose-200" : item.status === "Low" ? "bg-amber-50 text-amber-600 border-amber-200" : "bg-emerald-50 text-emerald-600 border-emerald-200")}>
                          {item.status}
                        </span>
                      </td>
                      {isAdmin && (
        ...
        <div className="space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            <div className="p-5 border-b border-slate-100">
              <h3 className="text-lg font-black tracking-tight text-slate-900">Order Drafts</h3>
              <p className="text-sm text-slate-500">Auto-suggested reorders</p>
            </div>
            <div className="max-h-[320px] overflow-y-auto p-4 space-y-4">
              {inventory.filter((i: any) => i.quantity_in_stock < (i.min_stock_level || 10)).length === 0 ? (
                ...
                inventory.filter((i: any) => i.quantity_in_stock < (i.min_stock_level || 10)).slice(0, 5).map((item: any) => (
                  <div key={item.id} className="p-4 rounded-xl border border-slate-200 bg-white hover:border-indigo-300 transition-all">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="px-2.5 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest border border-rose-200 bg-rose-50 text-rose-600">{item.quantity_in_stock} left</span>
                    </div>
                    {isAdmin && (
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
            ...
            <div className="p-6 space-y-5">
              <div>
                ...
                <Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. Whiteboard Markers" className="rounded-xl border-slate-200" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Category</label>
                  <div className="relative">
                    <Input value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} placeholder="e.g. Stationery" className="rounded-xl border-slate-200" list="category-suggestions" />
                ...
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">SKU</label>
                  <Input value={formData.sku} onChange={(e) => setFormData({ ...formData, sku: e.target.value })} placeholder="e.g. ST-MB01" className="rounded-xl border-slate-200" />
                </div>
              </div>
              <div className="border-t border-slate-100 pt-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <Input type="number" min="0" value={formData.quantity_in_stock} onChange={(e) => setFormData({ ...formData, quantity_in_stock: parseInt(e.target.value) || 0 })} className="rounded-xl border-slate-200" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Unit Price (₹)</label>
                    <Input type="number" min="0" step="0.01" value={formData.unit_price} onChange={(e) => setFormData({ ...formData, unit_price: parseFloat(e.target.value) || 0 })} className="rounded-xl border-slate-200" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Min Stock Level</label>
                    <Input type="number" min="0" value={formData.min_stock_level} onChange={(e) => setFormData({ ...formData, min_stock_level: parseInt(e.target.value) || 0 })} className="rounded-xl border-slate-200" />
                  </div>
                </div>
              </div>
              {formData.name && formData.unit_price > 0 && formData.quantity_in_stock > 0 && (
                <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3 flex items-center justify-between">
                  <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600">Estimated Value</span>
                  <span className="text-sm font-black text-emerald-700">{formatCurrency(formData.quantity_in_stock * formData.unit_price)}</span>
                </div>
              )}
              <div className="flex justify-end gap-3 pt-2 border-t border-slate-100">
                <button onClick={() => setDialogOpen(false)} className="h-11 rounded-xl border border-slate-200 text-slate-700 font-black text-[10px] uppercase tracking-widest px-6 hover:bg-slate-50 transition-all">Cancel</button>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white border border-slate-200 rounded-xl shadow-xl w-full max-w-sm mx-4 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-end gap-3">
                <button onClick={() => setDeleteConfirm(null)} className="h-11 rounded-xl border border-slate-200 text-slate-700 font-black text-[10px] uppercase tracking-widest px-6 hover:bg-slate-50 transition-all">Cancel</button>
