        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-lg font-black tracking-tight text-slate-900">Library Management</h2>
                        <p className="text-sm text-slate-500">Manage library catalog, inventory, and book issues</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-3 bg-white border border-slate-200 rounded-xl p-5">
                    <div className="flex justify-between items-start mb-4">
                        <div>
                            <h3 className="text-lg font-black tracking-tight text-slate-900">Circulation Overview</h3>
                            <p className="text-sm text-slate-500">Real-time book circulation</p>
                        </div>
                    ...
                    <div className="h-[200px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={circulationVelocity}>
                                ...
                                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: "#94a3b8", fontSize: 9, fontWeight: "900", letterSpacing: "0.2em" }} />
                                <YAxis hide />
                                <Tooltip cursor={{ fill: "rgba(16,185,129,0.03)" }} contentStyle={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: "12px", fontSize: "10px" }} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {filteredBooks.length === 0 ? (
                        <div className="col-span-full py-16 text-center">
                            ...
                            <p className="text-sm font-bold text-slate-500">No books found matching search.</p>
                        </div>
                    ) : (
                        filteredBooks.map((book) => (
                            <div key={book.id} className="bg-white border border-slate-200 rounded-xl p-5 hover:shadow-md transition-all flex flex-col">
                                <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="space-y-2 mb-4 flex-1">
                                    <h4 className="font-bold text-sm text-slate-900 leading-tight">{book.title}</h4>
                                    <p className="text-xs font-bold text-slate-500">Author: {book.author}</p>
                                </div>

                                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                                    <div>
                                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Category</p>
                                        <p className="text-xs font-bold text-slate-700">{book.category || "General"}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Available</p>
                                        <p className="font-bold text-sm text-slate-900">{book.available_copies} <span className="text-slate-400">/ {book.total_copies}</span></p>
                                    </div>
                                </div>
                            </div>
                        ))
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-slate-100">
                                    ...
                                    <th className="py-4 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 text-center">Student</th>
                                    <th className="py-4 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 text-center">Duration</th>
                                    <th className="py-4 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 text-center">Status</th>
                                    {isStaff && <th className="py-4 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 text-right">Action</th>}
                                </tr>
                            </thead>
                                ...
                                {transactions?.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="py-16 text-center">
                                            ...
                                            <p className="text-sm font-bold text-slate-500">No active book loans</p>
                                        </td>
                                    </tr>
                                ) : (
                                    transactions?.map((tx) => (
                                        <tr key={tx.id} className="hover:bg-slate-50/50 transition-colors">
                                            <td className="py-4 px-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <span className="font-bold text-sm text-slate-900">{tx.book?.title || "-"}</span>
                                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider">ISBN: {tx.book?.isbn || "-"}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-4 px-4 text-center">
                                                <span className="font-bold text-sm text-slate-700">{tx.student?.profile?.full_name}</span>
                                            </td>
                                            <td className="py-4 px-4 text-center">
                                                <div className="flex flex-col gap-1 text-xs font-bold">
                                                    <span className="text-slate-500">Issued: {tx.issue_date}</span>
                                                ...
                                                <td className="py-4 px-4 text-right">
                                                    {tx.status === "issued" && (
                                                        <button
                                                            ...
                                                            className="h-8 rounded-xl border border-slate-200 text-slate-700 font-black text-[9px] uppercase tracking-widest px-4 hover:bg-slate-50 transition-all"
                                                        >
                                                            Return
                                                        </button>
                                                    )}
            ...
            {activeTab === "inventory" && (
                <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-slate-100">
                                <th className="py-4 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 text-left">Item Name</th>
                                <th className="py-4 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 text-center">Category</th>
                                <th className="py-4 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 text-center">Stock Quantity</th>
                                <th className="py-4 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 text-right">Unit Price</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {inventoryItems?.length === 0 ? (
                                <tr><td colSpan={4} className="py-16 text-center"><p className="text-sm font-bold text-slate-500">No inventory items found</p></td></tr>
                            ) : (
                                inventoryItems?.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="py-4 px-4 text-center">
                                            <span className="text-sm font-bold text-slate-500">{item.category || "General"}</span>
                                        </td>
                                        <td className="py-4 px-4 text-center">
                                            <span className={cn(
                                                "px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest",
                                        ...
                                        <td className="py-4 px-4 text-right">
                                            <span className="font-bold text-sm text-slate-500 tabular-nums">₹{Number(item.unit_price).toLocaleString("en-IN")}</span>
                                        </td>
                                    </tr>
                                ))
                            )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-white border border-slate-200 rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
                        <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                            <h3 className="text-lg font-black tracking-tight text-slate-900">Issue Book</h3>
                            <button onClick={() => setIsIssueOpen(false)} className="h-8 w-8 rounded-xl hover:bg-slate-100 flex items-center justify-center">
                                <X className="h-4 w-4 text-slate-500" />
                            </button>
                        </div>
                        <div className="p-5 space-y-4">
                            <div>
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Select Book</label>
                                <select
                            ...
                            <div>
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Select Student</label>
                                <select
                            ...
                            <div>
                                ...
                                <Input type="date" value={issueForm.due_date} onChange={(e) => setIssueForm({ ...issueForm, due_date: e.target.value })} className="rounded-xl border-slate-200" />
                            </div>
                            <button
            ...
            {isAddBookOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-white border border-slate-200 rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
                        <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                            <h3 className="text-lg font-black tracking-tight text-slate-900">Add New Book</h3>
                            <button onClick={() => setIsAddBookOpen(false)} className="h-8 w-8 rounded-xl hover:bg-slate-100 flex items-center justify-center">
                                <X className="h-4 w-4 text-slate-500" />
                            </button>
                        </div>
                        <div className="p-5 space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Book Title</label>
                                    <Input value={bookForm.title} onChange={(e) => setBookForm({ ...bookForm, title: e.target.value })} placeholder="Title..." className="rounded-xl border-slate-200" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Author</label>
                                    <Input value={bookForm.author} onChange={(e) => setBookForm({ ...bookForm, author: e.target.value })} placeholder="Author..." className="rounded-xl border-slate-200" />
                                </div>
                            </div>
                            <div>
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">ISBN</label>
                                <Input value={bookForm.isbn} onChange={(e) => setBookForm({ ...bookForm, isbn: e.target.value })} placeholder="ISBN..." className="rounded-xl border-slate-200" />
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Category</label>
                                    <Input value={bookForm.category} onChange={(e) => setBookForm({ ...bookForm, category: e.target.value })} placeholder="General..." className="rounded-xl border-slate-200" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Total Copies</label>
                                    <Input type="number" value={bookForm.total_copies} onChange={(e) => setBookForm({ ...bookForm, total_copies: e.target.value })} className="rounded-xl border-slate-200" />
                                </div>
                            </div>
                            <div>
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-1.5">Shelf Location</label>
                                <Input value={bookForm.shelf_location} onChange={(e) => setBookForm({ ...bookForm, shelf_location: e.target.value })} placeholder="e.g. A-12" className="rounded-xl border-slate-200" />
                            </div>
                            <button
