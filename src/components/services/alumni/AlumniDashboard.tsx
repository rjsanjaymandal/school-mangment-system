    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="relative w-full max-w-sm">
                ...
                <Input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 rounded-xl border-slate-200 dark:border-slate-800" placeholder="Search alumni..." />
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {filteredAlumni.length === 0 ? (
                    <div className="col-span-full py-16 text-center">
                        ...
                        <p className="text-sm font-bold text-slate-500 dark:text-slate-400">No alumni records found.</p>
                    </div>
                ) : (
                    filteredAlumni.map((alumnus) => (
                        <div key={alumnus.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="font-bold text-sm text-slate-900 dark:text-white">{alumnus.first_name} {alumnus.last_name}</p>
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="px-2.5 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-800">Class of {alumnus.graduation_year}</span>
                                ...
                                {isAdminOrTeacher && (
                                    <div className="flex gap-1">
                                        <button onClick={() => openEditAlumnus(alumnus)} className="h-8 px-3 rounded-xl text-[9px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-900 transition-all">Edit</button>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
                <div className="flex items-center gap-3 mb-3">
                    ...
                    <h3 className="text-lg font-black tracking-tight text-slate-900 dark:text-white">Transition Intelligence</h3>
                </div>
                <p className="text-sm text-slate-500 dark:text-slate-400">Graduation recording now moves students into the heritage registry, and the alumni module also supports manual record creation, editing, and cleanup for legacy data.</p>
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
                        <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                            <h3 className="text-lg font-black tracking-tight text-slate-900 dark:text-white">Record Graduation</h3>
                            <button onClick={() => setGraduationOpen(false)} className="h-8 w-8 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-900 flex items-center justify-center">
                        ...
                        <form onSubmit={async (event) => { event.preventDefault(); await handleGraduate(new FormData(event.currentTarget)); }} className="p-5 space-y-4">
                            <div>
                                ...
                                <select name="studentId" className="w-full h-11 rounded-xl border border-slate-200 dark:border-slate-800 px-3 text-sm font-bold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 focus:border-blue-300 outline-none">
                                    <option value="" className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">Select student</option>
                                    {students.map((student) => (
                                        <option key={student.id} value={student.id} className="bg-white dark:bg-slate-900 text-slate-900 dark:text-white">{student.profile?.first_name} {student.profile?.last_name} ({student.admission_number})</option>
                                    ))}
                                </select>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 block mb-1.5">Graduation Year</label>
                                    <Input name="year" type="number" defaultValue={new Date().getFullYear()} className="rounded-xl border-slate-200 dark:border-slate-800" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 block mb-1.5">Profession</label>
                                    <Input name="profession" placeholder="Current profession" className="rounded-xl border-slate-200 dark:border-slate-800" />
                                </div>
                            </div>
                            <div>
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 block mb-1.5">Company</label>
                                <Input name="company" placeholder="Current company" className="rounded-xl border-slate-200 dark:border-slate-800" />
                            </div>
                            <div>
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 block mb-1.5">Achievements</label>
                                <textarea name="achievements" placeholder="Key achievements" className="w-full h-20 rounded-xl border border-slate-200 dark:border-slate-800 px-3 py-2 text-sm font-bold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-blue-300 outline-none resize-none" />
                            </div>
            ...
            {alumniOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
                        <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                            <h3 className="text-lg font-black tracking-tight text-slate-900 dark:text-white">{editingAlumni ? "Edit Alumni" : "Add Alumni"}</h3>
                            <button onClick={() => { setAlumniOpen(false); setEditingAlumni(null); }} className="h-8 w-8 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-900 flex items-center justify-center">
                                <svg className="h-4 w-4 text-slate-500 dark:text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                            </button>
                        </div>
                        <div className="p-5 space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <Input value={alumniForm.first_name} onChange={(e) => setAlumniForm({ ...alumniForm, first_name: e.target.value })} className="rounded-xl border-slate-200 dark:border-slate-800" />
                                </div>
                                <div>
                                    ...
                                    <Input value={alumniForm.last_name} onChange={(e) => setAlumniForm({ ...alumniForm, last_name: e.target.value })} className="rounded-xl border-slate-200 dark:border-slate-800" />
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 block mb-1.5">Graduation Year</label>
                                    <Input type="number" value={alumniForm.graduation_year} onChange={(e) => setAlumniForm({ ...alumniForm, graduation_year: e.target.value })} className="rounded-xl border-slate-200 dark:border-slate-800" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 block mb-1.5">Email</label>
                                    <Input value={alumniForm.email} onChange={(e) => setAlumniForm({ ...alumniForm, email: e.target.value })} className="rounded-xl border-slate-200 dark:border-slate-800" />
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 block mb-1.5">Phone</label>
                                    <Input value={alumniForm.phone} onChange={(e) => setAlumniForm({ ...alumniForm, phone: e.target.value })} className="rounded-xl border-slate-200 dark:border-slate-800" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 block mb-1.5">Profession</label>
                                    <Input value={alumniForm.current_profession} onChange={(e) => setAlumniForm({ ...alumniForm, current_profession: e.target.value })} className="rounded-xl border-slate-200 dark:border-slate-800" />
                                </div>
                            </div>
                            <div>
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 block mb-1.5">Company</label>
                                <Input value={alumniForm.company} onChange={(e) => setAlumniForm({ ...alumniForm, company: e.target.value })} className="rounded-xl border-slate-200 dark:border-slate-800" />
                            </div>
                            <div>
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 block mb-1.5">Achievements</label>
                                <textarea value={alumniForm.achievements} onChange={(e) => setAlumniForm({ ...alumniForm, achievements: e.target.value })} className="w-full h-20 rounded-xl border border-slate-200 dark:border-slate-800 px-3 py-2 text-sm font-bold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-blue-300 outline-none resize-none" />
                            </div>
