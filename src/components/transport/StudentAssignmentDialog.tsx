            ...
            <DialogTrigger asChild>
                <Button className="group relative rounded-none h-12 bg-primary/10 hover:bg-primary/20 text-primary font-black px-8 skew-x-[-12deg] transition-all duration-300 border border-primary/20 overflow-hidden">
                    <span className="relative z-10 skew-x-[12deg] flex items-center gap-3 uppercase tracking-[0.1em] text-xs">
                        <UserPlus className="h-4 w-4" /> Authorize Assignment
                    </span>
            ...
            <DialogContent className="p-0 border-none bg-[#050505]/95 backdrop-blur-3xl max-w-lg overflow-hidden ring-1 ring-primary/30 shadow-[0_0_50px_rgba(16,185,129,0.2)]">
                <div className="bg-primary/10 border-b border-primary/20 p-10 relative overflow-hidden">
                    ...
                    <DialogHeader className="relative z-10">
                        <DialogTitle className="font-black text-3xl italic uppercase tracking-tighter text-primary italic text-center">Assignment Protocol</DialogTitle>
                        <p className="text-primary/70 text-[10px] font-mono font-bold uppercase tracking-[0.4em] mt-3 italic text-center italic flex items-center justify-center gap-3 italic">
                            <span className="w-8 h-px bg-primary/30" /> Logistics Authorization Registry <span className="w-8 h-px bg-primary/30" />
                        </p>
                    </DialogHeader>
                </div>
                ...
                <div className="p-10 space-y-8 bg-black/40">
                    ...
                    <div className="space-y-3">
                        <Label className="text-[10px] font-black uppercase text-primary tracking-widest italic ml-1 text-primary/60">Subject Identification</Label>
                            ...
                            <PopoverTrigger asChild>
                                ...
                                >
                                    <span className="flex items-center gap-3">
                                        ...
                                        {selectedStudentId
                                            ...
                                            : fetchingStudents ? "SYNATPCING DATA..." : "IDENTIFY STUDENT..."}
                                    </span>
                             ...
                             <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0 bg-[#0a0a0a] border border-primary/30 shadow-[0_0_30px_rgba(0,0,0,0.5)] rounded-none">
                                <Command className="bg-transparent">
                                    <CommandInput placeholder="SCAN REGISTRY..." className="h-14 border-none focus:ring-0 font-black uppercase italic tracking-widest text-primary placeholder:text-primary/30" />
                                    <CommandList className="scrollbar-none max-h-[300px]">
                                        <CommandEmpty className="py-10 text-center text-[10px] font-black text-red-500/50 uppercase tracking-[0.3em] italic">Record not found in database.</CommandEmpty>
                    ...
                    <div className="grid grid-cols-1 gap-8">
                        <div className="space-y-3">
                            <Label className="text-[10px] font-black uppercase text-primary tracking-widest italic ml-1 text-primary/60">Deployment Route</Label>
                        ...
                        <div className="space-y-3">
                            <Label className="text-[10px] font-black uppercase text-primary tracking-widest italic ml-1 text-primary/60">Designated Terminal</Label>
                            <Select value={selectedStopId} onValueChange={setSelectedStopId} disabled={!selectedRouteId}>
                                <SelectTrigger className="rounded-none bg-primary/5 border-primary/20 h-14 font-black uppercase tracking-tight text-xs hover:bg-primary/10 hover:border-primary/40 transition-all italic text-foreground px-6 focus:ring-0 disabled:opacity-30">
                                    <SelectValue placeholder={selectedRouteId ? "SELECT NODE" : "AWAITING VECTOR"} />
                                </SelectTrigger>
                                <SelectContent className="bg-[#0a0a0a] border-primary/30 rounded-none">
                                    {filteredStops.length === 0 ? (
                                        <div className="py-8 px-6 text-[10px] font-black text-red-500/40 uppercase tracking-widest text-center italic">No nodes registered for this vector.</div>
                                    ) : (
                    ...
                    >
                        <span className="relative z-10">{loading ? "SYNCHRONIZING..." : "AUTHORIZE DEPLOYMENT"}</span>
