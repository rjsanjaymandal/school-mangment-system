      ...
      <div className="p-5 border-b border-slate-100">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <button onClick={prevMonth} className="h-8 w-8 rounded-xl border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-all">
              <ChevronLeft className="h-4 w-4 text-slate-600" />
            </button>
            <span className="text-sm font-black min-w-[140px] text-center text-slate-900">
              {monthNames[currentDate.getMonth()]} {currentDate.getFullYear()}
            </span>
            <button onClick={nextMonth} className="h-8 w-8 rounded-xl border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-all">
          ...
          {days.map((day, index) => {
            ...
            return (
              ...
              >
                {day && (
                  <>
                    <div className={cn("text-sm font-black", isToday ? "text-emerald-600" : "text-slate-700")}>
                      {day}
                    </div>
                    {dayEvents.length > 0 && (
                      <div className="space-y-0.5 mt-1">
                        {dayEvents.slice(0, 2).map((event) => (
                          <div
                            key={event.id}
                            className={cn("text-[10px] px-1 py-0.5 rounded truncate font-bold", eventColors[event.type] || "bg-slate-100 text-slate-700")}
                          >
                            {event.title}
                          </div>
                        ))}
                        {dayEvents.length > 2 && (
                          <div className="text-[10px] font-black text-slate-500">+{dayEvents.length - 2} more</div>
                        )}
                      </div>
                    )}
                  </>
            ...
            {getEventsForDate(selectedDate.getDate()).length === 0 ? (
              ...
              <div className="space-y-2">
                {getEventsForDate(selectedDate.getDate()).map(event => (
                  <div key={event.id} className="flex items-start gap-2 p-3 bg-slate-50 rounded-xl">
