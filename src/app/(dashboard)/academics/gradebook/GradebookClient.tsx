      ...
      <UnifiedPageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            {isAdminOrTeacher && (
              <button onClick={exportCsv} className="h-10 px-4 rounded-xl border border-slate-200 bg-white text-[10px] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-all flex items-center gap-2 shadow-sm">
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="p-4 border-b border-slate-100 flex flex-col md:flex-row gap-4 items-center">
          <div className="flex gap-2 flex-1">
            <select className="h-10 rounded-xl border border-slate-200 px-3 text-sm font-bold text-slate-700 bg-white focus:border-blue-300 outline-none" value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)}>
              {classes.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
            <select className="h-10 rounded-xl border border-slate-200 px-3 text-sm font-bold text-slate-700 bg-white focus:border-blue-300 outline-none" value={selectedSubject} onChange={(e) => setSelectedSubject(e.target.value)}>
          ...
          <div className="relative">
            ...
            <Input placeholder="Search students..." className="h-10 rounded-xl border-slate-200 pl-9 w-48" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          </div>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="text-[10px] font-black uppercase tracking-widest text-slate-500 py-3 px-4 text-left w-16">Rank</th>
                  <th className="text-[10px] font-black uppercase tracking-widest text-slate-500 py-3 px-4 text-left">Student</th>
                  {subjectExams.map(exam => <th key={exam.id} className="text-[10px] font-black uppercase tracking-widest text-slate-500 py-3 px-4 text-center min-w-[80px]">{exam.name.substring(0, 10)}</th>)}
                  <th className="text-[10px] font-black uppercase tracking-widest text-slate-500 py-3 px-4 text-center w-20">Avg</th>
                  <th className="text-[10px] font-black uppercase tracking-widest text-slate-500 py-3 px-4 text-center w-20">Grade</th>
                </tr>
              </thead>
              <tbody>
                {filteredData.map((entry) => (
                  <tr key={entry.studentId} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4 px-4 text-center"><span className="font-bold text-slate-800">{entry.average}</span></td>
              ...
              <div key={entry.studentId} className="p-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                ...
                <div className="flex items-center gap-6">
                  <div className="text-center">
                    <p className="text-xl font-black text-slate-800">{entry.average}%</p>
        ...
        {viewMode === "bulk" && (
          <div className="p-4 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div>
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider">Bulk Grade Entry</h3>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Enter marks for selected exam</p>
              </div>
              <div className="flex items-center gap-2">
                <select className="h-10 rounded-xl border border-slate-200 px-3 text-sm font-bold text-slate-700 bg-white focus:border-blue-300 outline-none" value={selectedExam} onChange={(e) => setSelectedExam(e.target.value)}>
                  <option value="">Select Exam</option>
                  {classExams.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}
                </select>
                <button onClick={() => setViewMode("matrix")} className="h-10 px-4 rounded-xl border border-slate-200 bg-white text-[10px] font-black uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-all">Cancel</button>
            ...
            <div className="space-y-2">
              ...
              {bulkEntries.map((entry) => (
                <div key={entry.studentId} className="grid grid-cols-3 gap-4 items-center py-2">
                  ...
                  <Input type="number" placeholder="Enter marks" min={0} max={100} className="h-10 rounded-xl border-slate-200" />
                </div>
              ))}
            </div>
          </div>
