    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mt-1">Create and manage online examinations</p>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800">
                    <th className="text-left py-4 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Exam Title</th>
                    ...
                    <th className="text-left py-4 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Date</th>
                    <th className="text-center py-4 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Duration</th>
                    ...
                    <th className="text-center py-4 px-4 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredExams.map(exam => (
                    <tr key={exam.id} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 transition-colors">
                      <td className="py-4 px-4 font-bold text-sm text-slate-900 dark:text-white">{exam.title}</td>
                      <td className="py-4 px-4 text-sm text-slate-600 dark:text-slate-400">{(exam as any).subjects?.name || "-"}</td>
                      <td className="py-4 px-4 text-sm text-slate-600 dark:text-slate-400">{(exam as any).classes?.name || "-"}</td>
                      <td className="py-4 px-4 text-sm text-slate-600 dark:text-slate-400">{exam.exam_date ? new Date(exam.exam_date).toLocaleDateString() : "-"}</td>
                      <td className="py-4 px-4 text-center text-sm text-slate-600 dark:text-slate-400">{exam.duration_minutes} min</td>
                      <td className="py-4 px-4 text-center text-sm text-slate-600 dark:text-slate-400">{exam.total_marks}</td>
        ...
        <ERPCard accentColor="emerald">
          <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center gap-4">
            ...
            <select value={questionTypeFilter} onChange={(e) => setQuestionTypeFilter(e.target.value)}
              className="h-10 rounded-xl border border-slate-200 dark:border-slate-800 px-3 text-[10px] font-bold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 focus:border-emerald-300 outline-none">
          ...
          {loading ? (
            ...
            <div className="space-y-3 p-6">
              {filteredQuestions.map(q => (
                <div key={q.id} className="border border-slate-100 dark:border-slate-800 rounded-xl p-4 hover:bg-slate-50/50 transition-colors">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-sm text-slate-900 dark:text-white">{q.question_text}</p>
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="inline-flex px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-800">{q.marks} marks</span>
                      </div>
                    </div>
