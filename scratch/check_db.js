...
async function main() {
  ...
  console.log('\n--- impersonation_logs COLUMNS ---');
  const resColumns = await client.query(`
    SELECT column_name, data_type, is_nullable
    FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'impersonation_logs';
  `);
  console.table(resColumns.rows);

  console.log('\n--- impersonation_logs FOREIGN KEYS ---');
  const resKeys = await client.query(`
    ...
    WHERE tc.constraint_type = 'FOREIGN KEY' AND tc.table_name='impersonation_logs';
  `);
